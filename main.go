...
func main() {
	...
	log_file, err := os.OpenFile("C:/Users/Public/Downloads/SimplyFire.txt", os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
	...
	entry_name.PlaceHolder = "Григорий"
	...
	entry_surname.PlaceHolder = "Веселов"
	...
	company_choose := widget.NewSelect(
		[]string{
			`АО "СИТРОНИКС"`,
			`АО "СИТРОНИКС АЙ ТИ"`,
			`ООО "СИТРОНИКС ПРОЕКТЫ"`,
			`ООО "СИТРОНИКС РАЗРАБОТКА"`,
			`ООО "СИТРОНИКС СПЕЙС"`,
			`ООО "СИТРОНИКС ЭЛЕКТРО"`,
			`ООО "СПУТНИКС"`,
			`ООО "ЭМПЕРИУМ"`,
		},
		func(s string) {
